...
std::optional<std::vector<Detection>> ParseDetections(HailoContext& hailo_context, int target_w, int target_h, cv::Mat& RGB_frame, const std::vector<std::string>& class_names) {
	...
	for (size_t class_id = 0; class_id < class_names.size(); ++class_id) {
        ...
        if(class_count == 0){
            ...
            // std::cout << detections[6] << std::endl; // 输出检测到的目标数量
            for (int box_index = NMS_index; box_index < NMS_index + class_count*5; box_index += 5) {
...
cv::Mat Stream_resize(cv::Mat BGR_frame) {
    // 检查图像是否为空
    if (BGR_frame.empty()) {
        return BGR_frame;
    }
    
    // 计算缩放比例（取较小的比例，确保完整显示）
    double kx = 640.0 / BGR_frame.cols;
    double ky = 640.0 / BGR_frame.rows;
    double scale = std::min(kx, ky);  // 取较小值，保证完整画面
    
    // 等比例缩放
    cv::Mat resized;
    cv::resize(BGR_frame, resized, cv::Size(), scale, scale, cv::INTER_LINEAR);
    
    // 创建640x640的黑色画布（3通道，与输入图像类型一致）
    cv::Mat result = cv::Mat::zeros(640, 640, BGR_frame.type());
    
    // 计算居中位置（将缩放后的图像放在画布中心）
    int x = (640 - resized.cols) / 2;
    int y = (640 - resized.rows) / 2;
